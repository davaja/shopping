...
public class Checkout implements BarcodeScanListener {
    ...
    private final LinkedHashMap<Product, Integer> discountedProducts = new LinkedHashMap<Product, Integer>();
	...
	private BigDecimal runningTotal() {
		...
		for (Product product : scannedProducts.keySet()) {
            ...
            {
                discountedProducts.put(product, count);
            }
	...
	public void paymentAccepted() {
		...
		for (Product product : scannedProducts.keySet()) {
            ...
            if (product.isProductDiscounted())
            {
                  int frequency = count / 3;
                  discountAmount = product.priceOf(frequency);
                  for (int i =0; i < frequency; i++)
                  {
                    printer.printReceiptDiscountLine(product, frequency, discountAmount);
                  }
